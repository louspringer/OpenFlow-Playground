...
class ArtifactAnalyzer:
    ...
    def _generate_recommendations(self) -> list[str]:
        ...
        if self.analysis.missing_domains:
            recommendations.append(
                f"📁 Domains defined in model but no artifacts found: {
                    ', '.join(self.analysis.missing_domains)
                }",
            )
        ...
        if self.analysis.missing_requirements:
            recommendations.append(
                f"📋 {
                    len(self.analysis.missing_requirements)
                } requirements are not traced to any artifacts. Consider implementing these requirements.",
            )
        ...
        if large_files:
            recommendations.append(
                f"📦 {
                    len(large_files)
                } files are larger than 1MB. Consider if these should be in version control.",
            )
