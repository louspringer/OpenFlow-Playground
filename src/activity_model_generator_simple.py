...
class PlantUMLGenerator:
    ...
    def __init__(self, server_url: str = "http://localhost:20075"):
        ...
        try:
            ...
            logger.warning(f"⚠️  PlantUML server not available: {e}")
            self.server = None
    ...
    def generate_activity_diagram(self, class_structure: Dict[str, Any], 
        ...
        # Try to generate diagram if server is available
        if self.server:
            try:
                ...
                logger.warning(f"⚠️  PlantUML diagram generation failed: {e}")
                logger.info(f"💾 PlantUML code available at: {code_path}")
                return code_path
        else:
            logger.info(f"💾 PlantUML code saved (server not available): {code_path}")
            return code_path
    ...
    def generate_sequence_diagram(self, class_structure: Dict[str, Any],
        ...
        if self.server:
            try:
                ...
                logger.warning(f"⚠️  Sequence diagram generation failed: {e}")
                return code_path
        else:
            return code_path
...
def main():
    ...
    parser.add_argument("--output", "-o", default="generated_models_simple", 
