        ...
        try:
            ...
            for i, line in enumerate(lines):
                ...
                if (
                    ...
                    and not line_content.startswith("return")
                ):
                    # End of dictionary, ensure it's properly returned
                    logger.info(
                        f"  🔧 Found dictionary end at line {i + 1}: {line_content}"
                    )
                    completed_lines.append(original_line)
                    if not any(
                        "return" in prev_line for prev_line in completed_lines[-3:]
                    ):
                ...
                if line_content.startswith(
                    '"operation_name"'
                ) or line_content.startswith('"nodes"'):
                    # This looks like an incomplete dictionary literal
                    logger.info(
                        f"  🔧 Found incomplete dictionary at line {i + 1}: {line_content}"
                    )
                ...
                if in_try_block and (
                    i == len(lines) - 1
                    or (i < len(lines) - 1 and lines[i + 1].strip().startswith("def"))
                ):
                    ...
                    completed_lines.append(
                        f'{" " * (try_indent + 4)}logger.error(f"❌ Error in {lines[0].split("(")[0].split()[-1]}: {{e}}")'
                    )
