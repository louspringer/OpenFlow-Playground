...
class VocabularyAligner(BaseReflectiveModule):
    ...
    def create_vocabulary_mapping(
        ...
        logger.info(
            f"✅ Created vocabulary mapping {mapping_key}: {len(mappings)} terms"
        )
        return True

    def align_vocabulary_ontologically(
        self, source_text: str, target_domain: str
    ) -> str:
        """Align vocabulary using ontological principles"""
        logger.info(
            f"🎯 Aligning vocabulary to domain {target_domain} using ontological principles"
        )
    ...
    def align_vocabulary_manually(
        self, source_text: str, target_domain: str, manual_mappings: Dict[str, str]
    ) -> str:
        """Align vocabulary using manual mappings"""
        logger.info(
            f"🎯 Aligning vocabulary to domain {target_domain} using manual mappings"
        )
        ...
        logger.info(
            f"✅ Vocabulary aligned to domain {target_domain} using manual mappings"
        )
        return aligned_text
    ...
    def validate_vocabulary(self, domain: str, vocabulary: List[str]) -> Dict[str, Any]:
        ...
        long_terms = [
            term for term in validation_result["valid_terms"] if len(term) > 50
        ]
        if long_terms:
            validation_result["warnings"].append(
                f"Found {len(long_terms)} very long terms"
            )
    ...
    def import_vocabulary(
        self, domain: str, vocabulary_data: Any, format: str = "json"
    ) -> bool:
