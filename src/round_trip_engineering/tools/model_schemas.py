...
class DomainInfo(BaseModel):
    ...
    patterns: List[str] = Field(
        default_factory=list, description="File patterns for domain"
    )
    content_indicators: List[str] = Field(
        default_factory=list, description="Content indicators"
    )
    ...
    requirements: List[str] = Field(
        default_factory=list, description="Domain requirements"
    )
    ...
    refactored_modules: List[str] = Field(
        default_factory=list, description="Refactored modules"
    )
    ...
    package_potential: Optional[Dict[str, Any]] = Field(
        None, description="Package potential info"
    )
...
class ProjectModel(BaseModel):
    ...
    domain_architecture: Dict[str, CategoryInfo] = Field(
        default_factory=dict, description="Domain architecture"
    )
    domains: Dict[str, DomainInfo] = Field(
        default_factory=dict, description="Domain definitions"
    )
    dependency_relationships: Dict[str, Any] = Field(
        default_factory=dict, description="Dependency relationships"
    )
    requirements_traceability: List[Dict[str, Any]] = Field(
        default_factory=list, description="Requirements traceability"
    )
    use_cases: List[Dict[str, Any]] = Field(
...
class LogEntry(BaseModel):
    ...
    target_model: Optional[str] = Field(
        None, description="Target model name"
    )  # Fixed: was model_name
