...
class WorkflowVisualizationGUI:
    ...
    def _render_dashboard(self):
        ...
        for component_name, component_info in self.components.items():
            with st.expander(f"{component_info['icon']} {component_name}", expanded=False):
                ...
                with col2:
                    if st.button(f"View {component_name.split(':')[0]}", key=f"view_{component_name}"):
    ...
    def _render_component_page(self, component_name: str):
        ...
        # Header
        st.markdown(f"<h1>{component_info['icon']} {component_name}</h1>", unsafe_allow_html=True)
    ...
    def _render_system_status(self):
        ...
        for component_name, component_info in self.components.items():
            try:
                if component_info['component'] == 'pydeps':
                    ...
                    result = subprocess.run(["uv", "run", "pydeps", "--help"], 
        ...
        st.markdown(f"<div class='{status_color}'><strong>{status_icon} System Status</strong></div>", 
                   unsafe_allow_html=True)
    ...
    def _display_control_flow_patterns(self, result: Dict[str, Any]):
        ...
        if 'patterns' in result:
            ...
            with col2:
                st.metric("Loops", result['patterns'].get('for_loops', 0) + result['patterns'].get('while_loops', 0))
            with col3:
        ...
        if diagram_files:
            for diagram in diagram_files:
                with st.expander(f"📄 {diagram.name}", expanded=False):
                    if diagram.suffix in ['.puml', '.mmd', '.dot']:
