...
class OntologicalTransformer:
    ...
    def transform_reverse_engineering_to_code_generation(
        self, reverse_engineering_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        validation = self.ontology.validate_transformation(
            reverse_engineering_data, FormatType.DICT
        )

        if not validation["valid"]:
            raise ValueError(
                f"Transformation validation failed: {validation['errors']}"
            )
    ...
    def _apply_ontological_transformation(
        self, data: Any, from_context: ContextType, to_context: ContextType
    ) -> Any:
        ...
        if isinstance(data, dict):
            ...
            for key, value in data.items():
                ...
                if isinstance(value, (dict, list)):
                    transformed_value = self._apply_ontological_transformation(
                        value, from_context, to_context
                    )
                else:
            ...
            if self._should_transform_list_to_dict(data, from_context, to_context):
                ...
                return [
                    self._apply_ontological_transformation(
                        item, from_context, to_context
                    )
                    for item in data
                ]
    ...
    def _map_vocabulary_key(
        self, key: str, from_context: ContextType, to_context: ContextType
    ) -> str:
        ...
        for from_concept in from_concepts:
            if key in from_concept.constraints.get("naming", ""):
                ...
                for to_concept in to_concepts:
                    if from_concept.name in to_concept.get_relationships(
                        RelationshipType.TRANSFORMS_FROM
                    ) or to_concept.name in from_concept.get_relationships(
    ...
    def _should_transform_list_to_dict(
        self, data: List, from_context: ContextType, to_context: ContextType
    ) -> bool:
        ...
        if (
            from_context == ContextType.REVERSE_ENGINEERING
            and to_context == ContextType.CODE_GENERATION
            and data
    ...
    def _transform_list_to_dict_by_name(
        ...
        for component in components_list:
            if isinstance(component, dict) and "name" in component:
                ...
                if name in components_dict:
                    print(
                        f"  ⚠️  Warning: Duplicate component name '{name}' - merging properties"
                    )
    ...
    def transform_model_file(self, input_file: str, output_file: str) -> bool:
        ...
        try:
            ...
            transformed_data = self.transform_reverse_engineering_to_code_generation(
                input_data
            )
    ...
    def validate_model_file(
        self, model_file: str, expected_context: ContextType
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            expected_format = (
                FormatType.DICT
                if expected_context == ContextType.CODE_GENERATION
                else FormatType.LIST
            )

            # Validate using ontology
            validation = self.ontology.validate_transformation(
                model_data, expected_format
            )
    ...
    def generate_transformation_report(
        ...
        report = {
            "transformation": {
                ...
                "timestamp": (
                    str(Path(input_file).stat().st_mtime)
        ...
        # Validate input file
        if os.path.exists(input_file):
            input_validation = self.validate_model_file(
                input_file, ContextType.REVERSE_ENGINEERING
            )
            report["validation"]["input"] = input_validation

        # Validate output file
        if os.path.exists(output_file):
            output_validation = self.validate_model_file(
                output_file, ContextType.CODE_GENERATION
            )
        ...
        if os.path.exists(input_file) and os.path.exists(output_file):
            ...
            report["statistics"] = {
                ...
                "compression_ratio": (
                    len(json.dumps(output_data)) / len(json.dumps(input_data))
                    if input_data
                    else 0
                ),
            }
        ...
        if (
            "input" in report["validation"]
            and not report["validation"]["input"]["valid"]
        ):
            report["recommendations"].append(
                "Fix input validation errors before transformation"
            )

        if (
            "output" in report["validation"]
            and not report["validation"]["output"]["valid"]
        ):
            report["recommendations"].append(
                "Review transformation logic - output validation failed"
            )

        if report["statistics"].get("compression_ratio", 1) > 1.5:
            report["recommendations"].append(
                "Output significantly larger than input - review transformation efficiency"
            )
...
def main():
    ...
    if os.path.exists(input_file):
        ...
        # Validate input
        input_validation = transformer.validate_model_file(
            input_file, ContextType.REVERSE_ENGINEERING
        )
        ...
        if success:
            # Validate output
            output_validation = transformer.validate_model_file(
                output_file, ContextType.CODE_GENERATION
            )
            ...
            print(
                f"  Input validation: {'✅ PASS' if input_validation['valid'] else '❌ FAIL'}"
            )
            print(
                f"  Output validation: {'✅ PASS' if output_validation['valid'] else '❌ FAIL'}"
            )
            print(
                f"  Input components: {report['statistics'].get('input_components', 'N/A')}"
            )
            print(
                f"  Output components: {report['statistics'].get('output_components', 'N/A')}"
            )
