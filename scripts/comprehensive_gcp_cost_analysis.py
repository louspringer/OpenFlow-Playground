...
class ComprehensiveGCPCostAnalyzer:
        ...
        for cluster in clusters:
            ...
            try:
                detail = self.run_gcloud_command([
                    "gcloud", "container", "clusters", "describe", cluster_name, f"--location={location}"
                ])
    ...
    def get_compute_instances(self) -> Dict[str, Any]:
        ...
        instances = self.run_gcloud_command([
        ...
        for cluster_name, cluster_info in cluster_details.items():
            if cluster_info.get("status") == "RUNNING":
                ...
                gke_costs[cluster_name] = {
                    "management_fee": {
                        "per_hour": management_fee_per_hour,
                        "daily": daily_management_fee,
                        "monthly": monthly_management_fee
                    },
                    "node_costs": {
                        "per_hour": total_node_cost_per_hour,
                        "daily": daily_node_cost,
                        "monthly": monthly_node_cost
                    },
                    "total": {
                        "per_hour": management_fee_per_hour + total_node_cost_per_hour,
    ...
    def calculate_firestore_costs(self, firestore_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for db_name, db_info in firestore_info.items():
            if db_info.get("free_tier", True):
                ...
                firestore_costs[db_name] = {
                    "tier": "Paid Tier",
                    "daily_cost": 0.17,  # Rough estimate
                    "monthly_cost": 5.0,
    ...
    def generate_daily_cost_breakdown(self) -> Dict[str, Any]:
        ...
        for db_name, db_cost in firestore_costs.items():
            daily_breakdown["firestore"][db_name] = {
                "daily_cost": db_cost["daily_cost"],
                "monthly_cost": db_cost["monthly_cost"],
                "tier": db_cost["tier"]
            }
            daily_breakdown["totals"]["firestore_daily"] += db_cost["daily_cost"]
        
        # Calculate totals
        daily_breakdown["totals"]["total_daily"] = (
            daily_breakdown["totals"]["gke_daily"] +
            daily_breakdown["totals"]["compute_daily"] +
            daily_breakdown["totals"]["firestore_daily"]
        )
