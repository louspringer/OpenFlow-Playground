...
from .models import (
    CacheConfig, CacheFormat, CacheHealth, CacheMetadata, 
    CacheOperation, CacheStats, CacheStatus
)
...
class CacheManager:
    ...
    def load_cache(self, cache_key: str, cache_file: Optional[Path] = None) -> Any:
        ...
        try:
            ...
            self._stats.avg_load_time_ms = (
                (self._stats.avg_load_time_ms + duration_ms) / 2
            )
        ...
        try:
            ...
            if success:
                ...
                self._stats.avg_save_time_ms = (
                    (self._stats.avg_save_time_ms + duration_ms) / 2
                )
    ...
    def _validate_cache_entry(self, cache_key: str, data: Any) -> bool:
        ...
        if data_size > self.config.max_size_mb * 1024 * 1024:
            return False
    ...
    def _update_metadata(self, cache_key: str, operation: str):
        ...
        if cache_key not in self._metadata:
            self._metadata[cache_key] = CacheMetadata(
                key=cache_key,
                value=self._cache_data[cache_key]
            )
    ...
    def _record_operation(self, cache_key: str, operation: str, success: bool, error_message: Optional[str] = None):
