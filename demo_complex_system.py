...
class AdvancedProcessor(DataProcessor):
    ...
    def process(self, data: List[DataItem]) -> List[DataItem]:
        ...
        if self.transform_func:
            data = [DataItem(id=item.id, value=self.transform_func(item.value), metadata=item.metadata) for item in data]
...
class ResultFormatter:
    ...
    def format_json(self, data: List[DataItem]) -> str:
        ...
        return json.dumps([{"id": item.id, "value": item.value, "metadata": item.metadata} for item in data], indent=2)
...
if __name__ == "__main__":
    ...
    processor = AdvancedProcessor(filter_func=lambda item: item.value is not None, transform_func=lambda value: str(value) if not isinstance(value, (int, float)) else value)
