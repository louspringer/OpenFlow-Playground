...
1. **📁 STEP 2**: Determine file type and build file header
1. **📚 STEP 3**: Analyze and generate smart imports
1. **🔍 STEP 4**: Detect advanced typing needs (dataclass, pydantic, enum)
1. **🏗️ STEP 5**: Generate all classes with methods and type hints
1. **⚙️ STEP 6**: Generate standalone functions with type hints
1. **🚀 STEP 7**: Generate main entry point and **main** guard
1. **✨ STEP 8**: Clean up and format the final generated code
...
1. **Detects Dependencies**: Automatically determines needed imports
1. **Preserves Structure**: Maintains exact class/method organization
1. **Generates Skeletons**: Creates TODO-filled implementation stubs
1. **Ensures Quality**: Produces valid, lintable Python code
1. **Supports Different File Types**: Handles scripts, packages, tests
...
1. **Feature Visibility**: All sophisticated features are documented
1. **Usage Examples**: Clear understanding of input/output expectations
1. **Maintenance**: Future developers can understand the 8-step process
1. **Quality Assurance**: Clear understanding of what "success" looks like
