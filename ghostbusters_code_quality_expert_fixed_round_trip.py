...
class CodeQualityExpert(BaseExpert):
    ...
    async def _analyze_existing_lint_output(self, project_path: Path) -> Any:
        """
    ...
    async def _identify_systemic_problems(self, linting_issues: Any) -> Any:
        """
        ...
        if len(import_errors) > len(linting_issues) * 0.3:
            systemic_issues.append(
                self._create_delusion(
                    ...
                    "project-wide",
                    1,
                    f"Widespread import organization issues: {len(import_errors)} import-related errors across {len(files_affected)} files",
                    0.9,
                    "high",
                )
            )
        ...
        if len(formatting_errors) > len(linting_issues) * 0.4:
            systemic_issues.append(
                self._create_delusion(
                    ...
                    "project-wide",
                    1,
                    f"Widespread code formatting issues: {len(formatting_errors)} formatting errors across {len(files_affected)} files",
                    0.85,
                    "medium",
                )
            )
        type_errors = [code for code in error_codes if code in ["F821", "F822", "F823"]]
        if len(type_errors) > len(linting_issues) * 0.2:
            systemic_issues.append(
                self._create_delusion(
                    "systemic_type_issues",
                    "project-wide",
                    1,
                    f"Widespread type annotation issues: {len(type_errors)} type-related errors across {len(files_affected)} files",
                    0.8,
                    "medium",
                )
            )
        if len(files_affected) > len(linting_issues) * 0.1:
            systemic_issues.append(
                self._create_delusion(
                    "widespread_quality_issues",
                    "project-wide",
                    1,
                    f"Quality issues spread across {len(files_affected)} files, indicating systemic process problems",
                    0.9,
                    "high",
                )
            )
    ...
    async def _generate_systemic_recommendations(self, linting_issues: Any) -> Any:
        """
        ...
        if import_errors:
            recommendations.append(
                self._create_recommendation(
                    f"Implement import sorting with isort: {len(import_errors)} import organization issues detected",
                    "high",
                )
            )
            recommendations.append(
        ...
        if formatting_errors:
            recommendations.append(
                self._create_recommendation(
                    f"Implement automated formatting with Black: {len(formatting_errors)} formatting issues detected",
                    "high",
                )
            )
            recommendations.append(
        ...
        if type_errors:
            recommendations.append(
                self._create_recommendation(
                    f"Implement gradual type annotation strategy: {len(type_errors)} type-related issues detected",
                    "high",
                )
            )
